...
const OverviewCard = () => {
    ...
    useEffect(() => {
        const fetchOverview = async () => {
            try {
                /**
                 * Retrieves sales performance metrics by calling the 'ms-crm' microservice.
                 */
     ...
     */
    const metrics = [
        { title: 'Won', value: formatK(data?.won), colorClass: "text-[#002D6B]" },
        { title: 'Pending', value: formatK(data?.pending), colorClass: "text-yellow-600" },
        { title: 'Lost', value: formatK(data?.lost), colorClass: "text-[#8F1600]" },
    ];
    ...
    return (
        <div className="bg-white rounded-xl shadow-xl overflow-hidden p-6 border border-gray-100 transition duration-300 hover:shadow-2xl">
            <CardTitle title="Revenue Summary" />
